...
export default function EditDevice() {
    ...
    async function saveLocation(e) {
        ...
        try {
            ...
            const response = await fetch(`${backendUrl}/device/update/${email}`, {
                method: "PUT", // You are updating something, so PUT is fine
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${jwtToken}`
                },
                body: JSON.stringify(formData), // Make sure formData is a plain JS object
                mode: "cors" // Required only if backend is on a different domain
            });
                    ...
                    <form className="space-y-5" onSubmit={saveLocation}>
                        ...
                        <div className="mt-8 mb-4">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} type="button" onClick={getLocation} disabled={loading} className={`flex items-center px-4 py-2 rounded-lg text-black font-bold  ${loading ? "bg-gray-400 cursor-not-allowed" : "bg-gradient-to-r from-blue-400 to-yellow-200 hover:from-yellow-200 hover:to-blue-400"}`}>
                                    {loading ? (<>
                                        {/* Green Dot Div */}
                                        <div className="animate-spin h-4 w-4 mr-2 border-2 border-white border-t-transparent rounded-full"></div>
                                        Detecting...</>
                                    ) : (
                                        <div className="">Detect Location</div>
                                    )}
                                </motion.button>
                            </div>
                        ...
                        {/* Register Button */}
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={loading || !userLocation}/*if it is loading or userlocation is not found disable the button*/ className={`w-full flex items-center justify-center p-3 rounded-lg text-white font-medium mt-6 ${loading || !userLocation ? "bg-gray-400 cursor-not-allowed" : "bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700"}`}>
                            {loading ? (
                                <>
